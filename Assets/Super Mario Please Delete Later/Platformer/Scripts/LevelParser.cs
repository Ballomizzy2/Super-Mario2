    ...
    void LoadLevel()
    {
        ...
        while (levelRows.Count > 0)
        {
            ...
            for (int col = 0; col < letters.Length; ++col)
            {
                ...
                // check for letter before spawning
                ...
                if (letters[col] == 'x')
                {
                    Vector3 pos = new Vector3(col + offset1, row + offset1, -2.0f);
                    GameObject newObj = Instantiate(rockPrefab, environmentRoot);
                    newObj.transform.position = pos;
                }
                else if(letters[col] == 's')
                {
                    Vector3 pos = new Vector3(col + offset2*2, row + offset2, -2.0f);
                ...
                else if (letters[col] == 'n')
                {
                    Vector3 pos = new Vector3(col + offset2 * 2, row + offset2, -2.0f);
                    GameObject newObj = Instantiate(dangerPrefab, environmentRoot);
                    newObj.transform.position = pos;
                }
                else if (letters[col] == 'f')
                {
                    Vector3 pos = new Vector3(col + offset2 * 2, row + offset2, -2.0f);
                    GameObject newObj = Instantiate(finishPrefab, environmentRoot);
                    newObj.transform.position = pos;
                }
            }
            row++;
